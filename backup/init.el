...
(defun my-custom-function ()
  "My custom function that prints a message."
  (interactive)(define-key global-map (kbd "C-c o") nil)
...
(defun my/open-org-file ()
  "Open an *.org file in the current directory, excluding hidden files."
  (interactive)
  (let ((org-files (directory-files default-directory nil "\\.org$"))
        (file))
    (if org-files
        (setq file (completing-read "Open org file: " org-files))
      (user-error "No *.org files found in this directory"))
    (find-file (expand-file-name file))))

(global-set-key (kbd "C-k") 'my/open-org-file)
(define-key global-map (kbd "C-o") nil)
(global-set-key (kbd "C-o") 'my/open-org-file)
